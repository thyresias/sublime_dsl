...
language 'Perl' => 'source.perl' do
  ...
  rule do
    ...
    from %r/\b(?=s([^\s\w\[({<]).*\1([egimosradlu]*)([\}\)\;\,]|\s+))/
    to %r/((([egimosradlu]*)))(?=([\}\)\;\,]|\s+|$))/,
      1 => 'string.regexp.replace.perl',
      2 => 'punctuation.definition.string.perl',
      3 => 'keyword.control.regexp-option.perl'
  ...
  rule do
    ...
    from %r/\b(?=(?<!\\)s\s*([^\s\w\[({<>]))/
    to %r/\2((([egimosradlu]*x[egimosradlu]*)))\b/,
      1 => 'string.regexp.replace.perl',
      2 => 'punctuation.definition.string.perl',
      3 => 'keyword.control.regexp-option.perl'
    ...
    rule 'string.regexp.replace.extended.simple_delimiter.perl' do
      from %r/'/
      to %r/'(?=[egimosradlu]*x[egimosradlu]*)\b/
      both 0 => 'punctuation.definition.string.perl'
      include "#escaped_char"
    end
    rule 'string.regexp.replace.extended.simple_delimiter.perl' do
      from %r/(.)/
      to %r/§1(?=[egimosradlu]*x[egimosradlu]*)\b/
      both 0 => 'punctuation.definition.string.perl'
      include "#escaped_char"
      include "#variable"
    end
  ...
  rule 'string.regexp.find.perl' do
    from %r'(?<!\\)((~\s*)?\/)(?=\S)(?=([^\\]|\\.)*\/)'
    to %r'(\/)'
    both 1 => 'punctuation.definition.string.perl'
    include "#escaped_char"
  end
  ...
  fragment :variable do
    ...
    rule 'variable.other.readwrite.global.perl' do
      # FIXME: (warning) character class has duplicated range
      match %r/([\$\@\%](#)?)([a-zA-Zx7f-xff\$]|::)([a-zA-Z0-9_x7f-xff\$]|::)*\b/,
        1 => 'punctuation.definition.variable.perl'
    end
    rule 'variable.other.readwrite.global.perl' do
      # FIXME: (warning) character class has duplicated range
      match %r/(\$\{)(?:[a-zA-Zx7f-xff\$]|::)(?:[a-zA-Z0-9_x7f-xff\$]|::)*(\})/,
        1 => 'punctuation.definition.variable.perl',
