...
language 'AppleScript' => 'source.applescript' do
  ...
  fragment :blocks do
    ...
    rule 'meta.function.positional.applescript' do
      ...
      from %r/^(?x)
        ...
        3 => 'punctuation.definition.parameters.begin.applescript',
        4 => 'variable.parameter.handler.applescript',
        5 => 'punctuation.definition.parameters.end.applescript'
      to %r/^\s*(end)(?:\s+(\2))?(?=\s*(--.*?)?$)/,
        1 => 'keyword.control.function.applescript'
      include "$self"
    end
  ...
  fragment :comments do
    rule 'comment.line.number-sign.applescript' do
      from %r/^\s*(#!)/
      to %r/\n/
      both 1 => 'punctuation.definition.comment.applescript'
    end
    rule do
      from %r/(^[ \t]+)?(?=--)/,
        1 => 'punctuation.whitespace.comment.leading.applescript'
      to %r/(?!\G)/
  ...
  fragment :"comments.nested" do
    rule 'comment.block.applescript' do
      from %r/\(\*/,
        0 => 'punctuation.definition.comment.begin.applescript'
      to %r/\*\)/,
        0 => 'punctuation.definition.comment.end.applescript'
      include "#comments.nested"
    end
  end
  ...
  fragment :"data-structures" do
    rule 'meta.array.applescript' do
      ...
      from %r/\{/,
        0 => 'punctuation.definition.array.begin.applescript'
      to %r/\}/,
        0 => 'punctuation.definition.array.end.applescript'
      rule do
        match %r/(\w+|((\|)[^|\n]*(\|)))\s*(:)/,
