...
language 'Diff' => 'source.diff' do
  ...
  rule 'meta.diff.header.git' do
    match %r'^diff --git a/.*$\n?'
  end
  rule 'meta.diff.header.command' do
    match %r/^diff (-|\S+\s+\S+).*$\n?/
  end
  ...
  rule 'meta.diff.index.git' do
    match %r/^index [0-9a-f]{7,40}\.\.[0-9a-f]{7,40}.*$\n?/
  end
