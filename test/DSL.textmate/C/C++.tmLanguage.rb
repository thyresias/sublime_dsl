...
language 'C++' => 'source.c++' do
  ...
  file_types %w(cc cpp cp cxx c++ C h hh hpp h++)
  ...
  rule 'storage.modifier.c++' do
    match %r/\b(constexpr|export|mutable|typename)\b/
  end
  ...
  fragment :special_block do
    rule 'meta.namespace-block${2:+.$2}.c++' do
      from %r/\b(namespace)\b\s*([_A-Za-z][_A-Za-z0-9]*\b)?+/,
        1 => 'storage.type.c++',
      ...
      rule do
        from %r/\{/
        to %r/\}/
        both 0 => 'punctuation.definition.scope.c++'
        include "#special_block"
        include "#constructor"
        include "$base"
      end
    ...
    rule 'meta.class-struct-block.c++' do
      from %r/\b(class|struct)\b\s*([_A-Za-z][_A-Za-z0-9]*\b)?+(\s*:\s*(public|protected|private)\s*([_A-Za-z][_A-Za-z0-9]*\b)((\s*,\s*(public|protected|private)\s*[_A-Za-z][_A-Za-z0-9]*\b)*))?/,
        1 => 'storage.type.c++',
        2 => 'entity.name.type.c++',
