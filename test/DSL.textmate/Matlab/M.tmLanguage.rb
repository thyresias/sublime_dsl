...
language 'MATLAB' => 'source.matlab', file: 'M' do
  file_types %w(m)
...
((?>[a-zA-Z_]\w*(?>[?!]|=(?!>))? )) # the function name/,
  ...
  fragment :all_matlab_comments do
    rule do
      ...
      to %r/(?!\G)/
      rule 'comment.line.double-percentage.matlab' do
        from %r/%%/,
          0 => 'punctuation.definition.comment.matlab'
        to %r/\n/
        rule do
          content_scope "meta.cell.matlab"
  ...
  fragment :matlab_keyword_control do
    rule 'keyword.control.matlab' do
      # Control keywords
      match %r/(?<!\.)\b(break|case|catch|continue|else|elseif|end|for|parfor|if|otherwise|pause|rethrow|return|start|startat|stop|switch|try|wait|while)\b/
    end
  end
