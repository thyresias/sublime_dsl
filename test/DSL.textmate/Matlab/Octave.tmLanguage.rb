...
language 'Octave' => 'source.octave' do
  file_types %w(m)
  key_equivalent "^~O"  # TextMate only
  uuid "236A240E-F4DA-45BA-905C-4046055E6247"  # TextMate only

  rule 'meta.function.with-arguments.octave' do
    content_scope "variable.parameter.input.function.octave"
    from %r/(?x)
...
\s*(\() # the opening parenthesis for arguments/,
      1 => 'storage.type.octave',
      2 => 'variable.parameter.output.function.octave',
      3 => 'variable.parameter.output.function.octave',
      4 => 'entity.name.function.octave'
    to %r/\)/,
      0 => 'punctuation.definition.parameters.octave'
  end
  rule 'meta.function.without-arguments.octave' do
    match %r/(?x)
(?=function\b) # borrowed from ruby bundle
(?<=^|\s)(function)\s+ # the function keyword
(?>\[(.*)\])? # match various different combination of output arguments
...
((?>[a-zA-Z_]\w*(?>[?!]|=(?!>))? )) # the function name/,
  ...
  fragment :all_octave_comments do
    rule do
      from %r/(^[ \t]+)?(?=%%)/,
        1 => 'punctuation.whitespace.comment.leading.octave'
      to %r/(?!\G)/
      rule 'comment.line.double-percentage.octave' do
        from %r/%%/,
          0 => 'punctuation.definition.comment.octave'
        to %r/\n/
      end
    end
    rule 'comment.block.percentage.octave' do
      from %r/%\{/
      to %r/%\}/
      both 1 => 'punctuation.definition.comment.octave'
    end
    rule do
      from %r/(^[ \t]+)?(?=%)/,
        1 => 'punctuation.whitespace.comment.leading.octave'
      to %r/(?!\G)/
      rule 'comment.line.percentage.octave' do
        from %r/%/,
          0 => 'punctuation.definition.comment.octave'
        to %r/\n/
      end
    ...
    rule do
      from %r/(^[ \t]+)?(?=#)/,
        1 => 'punctuation.whitespace.comment.leading.octave'
      to %r/(?!\G)/
      rule 'comment.line.number-sign.octave' do
        from %r/#/,
          0 => 'punctuation.definition.comment.octave'
        to %r/\n/
      end
  ...
  fragment :all_octave_keywords do
  ...
  fragment :brackets do
    rule do
      content_scope "meta.brackets.octave"
      from %r/\[/
      to %r/\]/
      both 0 => 'meta.brackets.octave'
      include "#allofem"
    end
  end

  fragment :constants_override do
    rule 'meta.inappropriate.octave' do
  ...
  fragment :curlybrackets do
    rule do
      content_scope "meta.brackets.curly.octave"
      from %r/\{/
      to %r/\}/
      both 0 => 'meta.brackets.curly.octave'
      include "#allofem"
      include "#end_in_parens"
    end
  end
  ...
  fragment :end_in_parens do
    rule 'keyword.operator.symbols.octave' do
  ...
  fragment :escaped_quote do
    rule 'constant.character.escape.octave' do
      match %r/''/
    end
  end

  fragment :not_equal_invalid do
    rule 'invalid.illegal.invalid-inequality.octave' do
  ...
  fragment :parens do
    rule do
      content_scope "meta.parens.octave"
      from %r/\(/
      to %r/\)/
      both 0 => 'meta.parens.octave'
      include "#allofem"
      include "#end_in_parens"
    end
  end
  ...
  fragment :string do
    rule 'string.quoted.single.octave' do
      from %r/((?<=(\[|\(|\{|=|\s|,|;))|^)'/,
        0 => 'punctuation.definition.string.begin.octave'
      to %r/'(?=(\]|\)|\}|=|~|<|>|&|\||-|\+|\*|\.|\^|\||\s|;|,))/,
        0 => 'punctuation.definition.string.end.octave'
  ...
  fragment :string_double do
    rule 'string.quoted.double.octave' do
      from %r/((?<=(\[|\(|\{|=|\s|;|:|,))|^)"/,
        0 => 'punctuation.definition.string.begin.octave'
      to %r/"(?=(\]|\)|\}|=|~|<|>|&|\||-|\+|\*|\.|\^|\||\s|;|:|,))/,
        0 => 'punctuation.definition.string.end.octave'
  ...
  fragment :transpose do
    rule 'keyword.operator.transpose.octave' do
      match %r/((\w+)|(?<=\])|(?<=\)))\.?'/
    end
  end

  fragment :unescaped_quote do
    rule 'invalid.illegal.unescaped-quote.octave' do
      match %r/'(?=.)/
    end
  end

  fragment :variable do
    rule 'variable.other.valid.octave' do
      # Valid variable.
  ...
  fragment :variable_assignment do
    rule 'invalid.illegal.incomplete-variable-assignment.octave' do
  ...
  fragment :variable_invalid do
    rule 'invalid.illegal.invalid-variable-name.octave' do
