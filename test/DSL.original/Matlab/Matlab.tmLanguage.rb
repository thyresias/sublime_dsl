...
language 'MATLAB' => 'source.matlab', file: 'Matlab' do
  file_types %w(matlab)
  folding_start_marker %r/^\s*(function|if|switch|while|for|try)\b(?!.*\bend\b).*$/
  folding_stop_marker %r/^\s*(end|return)\b.*$/
  key_equivalent "^~M"  # TextMate only
  uuid "48F8858B-72FF-11D9-BFEE-000D93589AF6"  # TextMate only

  rule 'meta.function.with-arguments.matlab' do
    content_scope "variable.parameter.input.function.matlab"
    from %r/(?x)
...
\s*(\() # the opening parenthesis for arguments/,
      ...
      2 => 'variable.parameter.output.function.matlab',
      3 => 'variable.parameter.output.function.matlab',
      4 => 'entity.name.function.matlab'
    to %r/\)/,
      0 => 'punctuation.definition.parameters.matlab'
  end
  rule 'meta.function.without-arguments.matlab' do
    match %r/(?x)
(?=function\b) # borrowed from ruby bundle
(?<=^|\s)(function)\s+ # the function keyword
(?>\[(.*)\])? # match various different combination of output arguments
...
((?>[a-zA-Z_]\w*(?>[?!]|=(?!>))? )) # the function name/,
  ...
  fragment :all_matlab_comments do
    rule 'comment.double.percentage.matlab' do
      match %r/(%%).*$\n?/,
        1 => 'punctuation.definition.comment.matlab'
    end
    rule 'comment.block.percentage.matlab' do
      from %r/%\{/
      to %r/%\}\s*\n/
      both 1 => 'punctuation.definition.comment.matlab'
    end
    rule 'comment.line.percentage.matlab' do
      match %r/(%).*$\n?/,
        1 => 'punctuation.definition.comment.matlab'
    end
  end

  fragment :all_matlab_keywords do
  ...
  fragment :number do
    rule 'constant.numeric.matlab' do
      # Valid numbers: 1, .1, 1.1, .1e1, 1.1e1, 1e1, 1i, 1j, 1e2j
      match %r'(?<=[\s\-\+\*\/\\=:\[\(\{,]|^)\d*\.?\d+([eE][+-]?\d)?([0-9&&[^\.]])*(i|j)?\b'
    end
  end
  ...
  fragment :parens do
    rule do
      content_scope "meta.parens.matlab"
      from %r/\(/
      to %r/\)/
      both 0 => 'meta.parens.matlab'
      include "#allofem"
      include "#end_in_parens"
    end
  end

  fragment :special_characters do
    rule 'constant.character.escape.matlab' do
      # Operator symbols
      match %r/((\%([\+\-0]?\d{0,3}(\.\d{1,3})?)(c|d|e|E|f|g|G|s|((b|t)?(o|u|x|X))))|\%\%|\\(b|f|n|r|t|\\))/
    end
  end

  fragment :string do
    rule 'string.quoted.single.matlab' do
      from %r/((?<=(\[|\(|\{|=|\s|;|:|,))|^)'/,
  ...
  fragment :transpose do
    rule 'keyword.operator.transpose.matlab' do
      match %r/((\w+)|(?<=\])|(?<=\)))\.?'/
    end
  end

  fragment :unescaped_quote do
    rule 'invalid.illegal.unescaped-quote.matlab' do
      match %r/'(?=.)/
    end
  end

  fragment :variable do
    rule 'meta.variable.other.valid.matlab' do
      # Valid variable. Added meta to disable hilightinh
      match %r/\b[a-zA-Z]\w*\b/
    end
  end

  fragment :variable_assignment do
    rule 'invalid.illegal.incomplete-variable-assignment.matlab' do
      # Incomplete variable assignment.
      match %r/=\s*\.{0,2}\s*;?\s*$\n?/
    end
  end

  fragment :variable_invalid do
    rule 'invalid.illegal.invalid-variable-name.matlab' do
      # No variables or function names can start with a number or an underscore.
      match %r/\b(_\w|\d+[_a-df-zA-DF-Z])\w*\b/
    end
  end
end
