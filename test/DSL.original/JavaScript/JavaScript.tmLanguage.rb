...
language 'JavaScript' => 'source.js' do
  ...
  first_line_match %r'^#!/usr/bin/env node'
  folding_start_marker %r/^.*\bfunction\s*(\w+\s*)?\([^\)]*\)(\s*\{[^\}]*)?\s*$/
  folding_stop_marker %r/^\s*\}/
  key_equivalent "^~J"  # TextMate only
  uuid "93E017CC-6F27-11D9-90EB-000D93589AF6"  # TextMate only

  rule 'comment.line.js' do
    # node.js shebang
    match %r'^#!/usr/bin/env node'
  end
  rule 'meta.class.js' do
    # match stuff like: Sound.prototype = { … } when extending an object
    match %r/([a-zA-Z_?.$][\w?.$]*)\.(prototype)\s*(=)\s*/,
      1 => 'support.class.js',
      2 => 'support.constant.js',
      3 => 'keyword.operator.js'
  end
  rule 'meta.function.prototype.js' do
    # match stuff like: Sound.prototype.play = function() { … }
    match %r/([a-zA-Z_?.$][\w?.$]*)\.(prototype)\.([a-zA-Z_?.$][\w?.$]*)\s*(=)\s*(function)?\s*(\()(.*?)(\))/,
      1 => 'support.class.js',
      2 => 'support.constant.js',
  ...
  rule 'meta.function.js' do
    ...
    match %r/([a-zA-Z_?.$][\w?.$]*)\.(prototype)\.([a-zA-Z_?.$][\w?.$]*)\s*(=)\s*/,
      1 => 'support.class.js',
      2 => 'support.constant.js',
      3 => 'entity.name.function.js',
      4 => 'keyword.operator.js'
  ...
  rule 'meta.function.js' do
    ...
    match %r/([a-zA-Z_?.$][\w?.$]*)\.([a-zA-Z_?.$][\w?.$]*)\s*(=)\s*(function)\s*(\()(.*?)(\))/,
      1 => 'support.class.js',
  ...
  rule 'meta.function.js' do
    ...
    match %r/([a-zA-Z_?$][\w?$]*)\s*(=)\s*(function)\s*(\()(.*?)(\))/,
  ...
  rule 'meta.function.js' do
    ...
    match %r/\b(function)\s+([a-zA-Z_$]\w*)?\s*(\()(.*?)(\))/,
  ...
  rule 'meta.function.json.js' do
    # match stuff like: foobar: function() { … }
    match %r/\b([a-zA-Z_?.$][\w?.$]*)\s*:\s*\b(function)?\s*(\()(.*?)(\))/,
      1 => 'entity.name.function.js',
  ...
  rule 'entity.name.type.object.js.firebug' do
    match %r/\b(console)\b/
  end
  rule 'support.function.js.firebug' do
    match %r/\.(warn|info|log|error|time|timeEnd|assert)\b/
  end
  ...
  rule 'storage.type.js' do
    match %r/\b(boolean|byte|char|class|double|enum|float|function|int|interface|long|short|var|void)\b/
  end
  rule 'storage.modifier.js' do
    match %r/\b(const|export|extends|final|implements|native|private|protected|public|static|synchronized|throws|transient|volatile)\b/
  end
