...
language 'Lua' => 'source.lua' do
  ...
  file_types %w(lua)
  folding_start_marker %r/^\s*\b(function|local\s+function|if|for)\b|{[ \t]*$|\[\[/
  ...
  rule 'support.function.lua' do
    match %r/(?<![^.]\.|:)\b(assert|collectgarbage|dofile|error|getfenv|getmetatable|ipairs|loadfile|loadstring|module|next|pairs|pcall|print|rawequal|rawget|rawset|require|select|setfenv|setmetatable|tonumber|tostring|type|unpack|xpcall)\b(?=[( {])/
  end
