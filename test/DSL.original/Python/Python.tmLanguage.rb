...
language 'Python' => 'source.python' do
  ...
  #
  file_types %w(py rpy pyw cpy SConstruct Sconstruct sconstruct SConscript)
  first_line_match %r'^#!/.*\bpython[0-9.-]*\b'
  folding_start_marker %r/^\s*(def|class)\s+([.a-zA-Z0-9_ <]+)\s*(\((.*)\))?\s*:|\{\s*$|\(\s*$|\[\s*$|^\s*"""(?=.)(?!.*""")/
  folding_stop_marker %r/^\s*$|^\s*\}|^\s*\]|^\s*\)|^\s*"""\s*$/
