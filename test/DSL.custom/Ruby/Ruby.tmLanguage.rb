...
language 'Ruby' => 'source.ruby' do
  ...
  rule 'invalid.deprecated.ruby' do
    # else if is a common mistake carried over from other languages.
    # it works if you put in a second end, but it’s never what you want.
    match %r/(?<!\.)\belse(\s)+if\b/
  end
  rule 'keyword.control.ruby' do
    # everything being a reserved word, not a value and needing a 'end'
  ...
  # here-docs:
  # interpolated like <<-XML or <<-TEXT_XML or <<-"XML"
  # non-interpolated like <<-'XML', etc.

  def heredoc(language, options = {})

    scope = options[:scope] || "source.#{language}"
    marker = options[:marker] ||language.upcase

    rule "string.unquoted.interpolated.ruby" do
      content_scope "text.#{language}.embedded.ruby"
      from %r/(?><<-("?)((?:\w+_|)#{marker})\b\1)/,
        0 => 'punctuation.definition.string.begin.ruby'
      to %r/\s*§2$/,
        0 => 'punctuation.definition.string.end.ruby'
      include "#heredoc"  # FIXME: simplistic
      include scope
      include "#interpolated_ruby"
      include "#escaped_char"
    end

    rule "string.unquoted.not-interpolated.ruby" do
      content_scope "text.#{language}.embedded.ruby"
      from %r/(?><<-'((?:\w+_|)#{marker})')/,
        0 => 'punctuation.definition.string.begin.ruby'
      to %r/\s*§1$/,
  ...
  heredoc 'html', scope: 'text.html.basic'
  heredoc 'xml', scope: 'text.xml'
  heredoc 'sql'
  heredoc 'sas'
  heredoc 'css'
  heredoc 'c++', marker: 'CPP'
  heredoc 'c'
  heredoc 'js', marker: '(?:JS|JAVASCRIPT)'
  heredoc 'js.jquery', marker: 'JQUERY'
  heredoc 'shell', marker: '(?:SH|SHELL)'
  heredoc 'ruby'

  # generic interpolated
  rule 'string.unquoted.interpolated.ruby' do
    from %r/(?><<-("?)(\w+)\b\1)/,
      0 => 'punctuation.definition.string.begin.ruby'
    to %r/\s*§2$/,
  ...
  fragment :regex_sub do
    ...
    rule 'comment.line.number-sign.ruby' do
      ...
      match %r/(?<=^|\s)(#)\s.*$/, 1 => 'punctuation.definition.comment.ruby'
    end
  end
end
