...
language 'PHP' => 'source.php' do
  ...
  fragment :constants do
    ...
    rule 'constant.other.php' do
      ...
      match %r/[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*/
    end
  end
  ...
  fragment :instantiation do
    rule do
      # FIXME: (error) invalid hex escape
      match %r/(?i)\b(new)\s+(?:(\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)|(\w+))|(\w+)(?=::)/,
  ...
  fragment :interpolation do
    ...
    rule do
      ...
      # FIXME: (error) invalid hex escape
      from %r/(?=(?<regex>(?#simple syntax)\$(?<name>[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)(?:\[(?<index>[a-zA-Z0-9_\x7f-\xff]+|\$\g<name>)\]|->\g<name>(\(.*?\))?)?|(?#simple syntax with braces)\$\{(?:\g<name>(?<indices>\[(?:\g<index>|'(?:\\.|[^'\\])*'|"(?:\g<regex>|\\.|[^"\\])*")\])?|\g<complex>|\$\{\g<complex>\})\}|(?#complex syntax)\{(?<complex>\$(?<segment>\g<name>(\g<indices>*|\(.*?\))?)(?:->\g<segment>)*|\$\g<complex>|\$\{\g<complex>\})\}))\{/
      to %r/\}/
  ...
  fragment :language do
    ...
    rule 'meta.catch.php' do
      # FIXME: (error) invalid hex escape
      match %r/\b(catch)\b\s*\(\s*([A-Za-z_][A-Za-z_0-9]*)\s*((\$+)[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\s*\)/,
    ...
    rule 'meta.function.php' do
      ...
      rule 'meta.function.argument.no-default.php' do
        # FIXME: (error) invalid hex escape
        match %r/(\s*&)?\s*((\$+)[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\s*(?=,|\))/,
          1 => 'storage.modifier.php',
          2 => 'variable.other.php',
      ...
      rule 'meta.function.argument.default.php' do
        # FIXME: (error) invalid hex escape
        from %r/(\s*&)?\s*((\$+)[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)(?:\s*(=)\s*)\s*/
        to %r/(?=,|\))/
    ...
    rule do
      # FIXME: (error) invalid hex escape
      match %r/(?<=[a-zA-Z0-9_\x7f-\xff])(->)([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*?)\b/,
